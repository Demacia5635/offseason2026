...
public class DemaciaKinematics {
    ...
    private ChassisSpeeds limitVelocities(ChassisSpeeds wantedSpeeds, ChassisSpeeds currentSpeeds) {
        ...
        return new ChassisSpeeds(currentVel.getX() + deltaV.getX(), currentVel.getY() + deltaV.getY(),
    ...
    final double maxFastTurnAngle = maxRadialA / maxLinearA; // if heading change is lower than this value - don't slow
                                                             // - accelerate to target velocity
    final double minReverseAngle = Math.PI - maxFastTurnAngle; // if heading is bigger than this - deaccelerate and turn
                                                               // to reverse (optimize)
    ...
    private ChassisSpeeds limitVelocitiesUdi(ChassisSpeeds wantedSpeeds, ChassisSpeeds currentSpeeds) {
        ...
        LogManager.log("currnet v: " + currentV + " wanted v: " + wantedV + " target v: " + targetV + " alpha: " + Math.toDegrees(alpha)
                + " target angle: " + Math.toDegrees(newTargetAngle) + " current angle: " + Math.toDegrees(currentAngle) + " target angle pre: " + Math.toDegrees(targetAngle));
        
        // return the speeds - using target velocity and target angle
        return new ChassisSpeeds(targetV * Math.cos(newTargetAngle), targetV * Math.sin(newTargetAngle),
                wantedSpeeds.omegaRadiansPerSecond);
    }
